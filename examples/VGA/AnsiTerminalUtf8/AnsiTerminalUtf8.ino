//!!!!!!!!!!!!
//Warning! The latest version of the Espressif ESP32 library that FabGL runs on is 2.0.17 (or even earlier). 
//!!!!!!!!!!!!
...
ufabgl::BitmappedDisplayController * DisplayController;
ufabgl::PS2Controller                PS2Controller;
ufabgl::Terminal                     Terminal;
ufabgl::SerialPort                   SerialPort;
ufabgl::SerialPortTerminalConnector  SerialPortTerminalConnector;
...
void setup()
{
  ...
  disableCore1WDT(); //hlasi chybu ze nejde odstranit, ale nezda sa ze by to niecomu vadilo

  // more stack is required for the UI (used inside Terminal.onVirtualKey)
  Terminal.keyboardReaderTaskStackSize = 3000;

  Terminal.inputQueueSize = 4096; // 2048 good to pass vttest
  ...
  ufabgl::Mouse::quickCheckHardware();
  ...
  if (ConfDialogApp::getBootInfo() == BOOTINFO_ENABLED) {
    Terminal.write("* *  uFabGL - Serial Terminal                            * *\r\n");
