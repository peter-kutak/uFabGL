
#pragma once



namespace ufabgl {


const uint8_t infoBitmap_data[] = {
  ,
};
const Bitmap infoBitmap = Bitmap(16, 16, &infoBitmap_data[0], PixelFormat::RGBA2222);



const uint8_t errorBitmap_data[] = {
  ,
};
const Bitmap errorBitmap = Bitmap(16, 16, &errorBitmap_data[0], PixelFormat::RGBA2222);



const uint8_t questionBitmap_data[] = {
  ,
};
const Bitmap questionBitmap = Bitmap(16, 16, &questionBitmap_data[0], PixelFormat::RGBA2222);



const uint8_t warnBitmap_data[] = {
  ,
};
const Bitmap warnBitmap = Bitmap(16, 16, &warnBitmap_data[0], PixelFormat::RGBA2222);



}
